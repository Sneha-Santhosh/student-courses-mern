...
exports.create = function (req, res) {
    ...
    console.log("in exports.create courses",req.body);
    //
    //
    Student.findOne({firstName: req.body.firstName}, (err, student) => {
...
exports.list = function (req, res) {
    Course.find().sort('-courseCode').populate('courseCode', 'firstName lastName fullName').exec((err, courses) => {
if (err) {
        return res.status(400).send({
            message: getErrorMessage(err)
        });
...
exports.courseByID = function (req, res, next, id) {
    Course.findById(id).populate('courseCode', 'firstName lastName fullName').exec((err, course) => {if (err) return next(err);
